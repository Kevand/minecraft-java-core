...
export default class Launch extends EventEmitter {
  ...
  async DownloadGame() {
    ...
    const gameAssetsOther: any = await libraries.GetAssetsOthers(
      this.options.url,
    );
    const gameAssets: any = await new assetsMinecraft(this.options).getAssets(
      json,
    );
    await new loggingMinecraft(this.options).getLogging(json);
    const gameJava: any = this.options.java.path
      ? { files: [] }
      : await java.getJavaFiles(json);

    if (gameJava.error) return gameJava;

    const filesList: any = await bundle.checkBundle([
      ...gameLibraries,
      ...gameAssetsOther,
      ...gameAssets,
      ...gameJava.files,
    ...
    if (this.options.loader.enable === true) {
      ...
      const jsonLoader = await loaderInstall
        .GetLoader(
          version,
